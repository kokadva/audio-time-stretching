...
def set_speed(wav, speed=0.5):
    ...
    """
        ...
        overlaping (size of overlaping would be CHUNK_SIZE * speed, size of the first chunk simply CHUNK_SIZE and 
        the quantity of the other chunks besides the first chunks_num - 1 plus we use each one twise so we just 
        multiply that value by two
    """
    samples_num = int(CHUNK_SIZE + (2 * chunks_num - 1) * CHUNK_SIZE * speed)
    ...
    """
        ...
        for it would be 0, then CHUNK_SIZE / SPEED, then 2 * (CHUNK_SIZE / SPEED) and on the X'th time it'll be 
        X * (CHUNK_SIZE / SPEED) so this X will be our overlap_index
    """
    overlap_index = 0
